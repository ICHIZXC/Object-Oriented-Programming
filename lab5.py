...
class Bank:
    ...
    def search_account_by_card(self, card):
        for user in self.__user_list:
            for acc in user.account_list:
                if acc.card == card:
                    return acc
        return None
...
class Account:
    ...
    def deposit(self, channel_id, amount):
        ...
        self.__transaction.append(Transaction("D", channel_id, amount, self.__balance))
        return "Success"
    ...
    def withdraw(self, channel_id, amount):
        ...
        self.__balance -= amount
        self.__transaction.append(Transaction("W", channel_id, amount, self.__balance))
        return "Success"
    
    def transfer(self, channel_id, amount, target_account):
        if amount <= 0:
            return "Error : amount must be greater than 0"
        if self.__balance < amount:
            return "Error : not enough money"
        self.__balance -= amount
        target_account.deposit(channel_id, amount)
        self.__transaction.append(Transaction("T", channel_id, amount, self.__balance))
        return "Success"
    
    def list_transaction(self):
        return self.__transaction
    
    def calculate_interest(self, year, rate):
        interest = self.__balance * rate * year
        self.__transaction.append(Transaction("I", "", interest, self.balance))
...
class SavingAccount(Account):
    def __init__(self, account_no, user, balance):
        ...
        self.__withdrawal_limit = 50000
        
    def withdraw(self, channel_id, amount):
        if amount > self.__withdrawal_limit:
            return "Error : over withdrawal limit"
        return super().withdraw(channel_id, amount)
        
    def calculate_interest(self, years):
        interest = self.balance * 0.005 * years
        self.balance += interest
        self.list_transaction().append(Transaction("I", "", interest, self.balance))
        return interest
        
class FixedAccount(Account):
    def __init__(self, account_no, user, duration, balance = 0):
        super().__init__(account_no, user, balance)
    ...
    def deposit(self, channel_id, amount):
        if self.balance == 0:
            self.__deposit_date = datetime.now()
        return super().deposit(channel_id, amount)
    
    def withdraw(self, channel_id, amount):
        from datetime import datetime
        if self.balance == 0:
            return "Error: No initial deposit"
        if (datetime.now() - self.__deposit_date).days < int(self.__duration)*30:
            self.__rate = 0.0125
        super().list_transaction().append(Transaction("I", "", self.balance*self.__rate, self.balance))
        super().calculate_interest(1, self.__rate)
        return super().withdraw(channel_id, amount)
    
    def calculate_interest(self, year):
        return super().calcuate_interset(year, self.__rate)
...
class EDCMachine(TransactionChannel):
    ...
    def pay(self, card, amount):
        account  = self.bank.search_account_by_card(card)
        amount - self.calculate_cashback(card, amount)
        if not isinstance (card, DebitCard):
            return "Error: No card inserted"
        return account.transfer(self.channel_id, amount, self.__merchant_account)
...
class BankingTest(unittest.TestCase):
    ...
    def test_withdraw_over_limit(self): # 3. ทดสอบการถอนเงินเกินจำนวนที่กำหนด
        ...
        self.assertIn("Error", withdraw_result, "Should return error for withdrawal over limit")
